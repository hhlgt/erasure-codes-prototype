...
namespace ECProject
{
  MainRecalResp Proxy::main_recal(MainRecalPlan recal_plan)
  {
    ...
    if (recal_plan.ec_type == Hierachical_PC) {
      ...
      ec->perform_addition(t_data, t_coding, block_size, data_num, parity_num);
      gettimeofday(&end_time, NULL);
      double computing_time = end_time.tv_sec - start_time.tv_sec +
      ...
      if (num_of_original_blocks > 0 && if_partial_decoding) {  // encode-and-transfer
        ...
        gettimeofday(&start_time, NULL);
        ec->encode_partial_blocks_for_encoding(data, coding, block_size,
      ...
      if (if_partial_decoding){
        ...
        ec->encode_partial_blocks_for_encoding(t_data, t_coding, block_size,
            *original_blocks_idx_ptr, *parity_idx_ptr);
      }
      gettimeofday(&end_time, NULL);
      computing_time += end_time.tv_sec - start_time.tv_sec +
  ...
  void Proxy::help_recal(HelpRecalPlan recal_plan)
  {
    ...
    if (t_flag) {
      ...
      std::vector<unsigned char> int_buf_num_of_blocks = int_to_bytes(parity_num);
      asio::write(socket_,
